...
for t in range(args.tries):
    if args.use_posteriors:
        ...
        model_dir = "models/image_student_posteriors/%s_%s_%s/" %(model_type, n_topics, t)
    elif args.use_priors:
        model_dir = "models/image_student_priors/%s_%s_%s/" %(model_type, n_topics, t)
    else:
    ...
    if args.use_posteriors:
        ...
        posterior_variance, posterior_mean, posterior_log_variance = get_posteriors(teacher_dataset, args.text_model)
        ...
        vctm = ZeroShotTM(bow_size=len(image_training_dataset.idx2token), contextual_size=512, n_components=int(n_topics), model_type=model_type, learn_priors=False)
